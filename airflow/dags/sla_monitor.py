    ...
    def check_freshness():
        ...
        try:
            ...
            with eng.begin() as c:
                c.execute(
                    text(
                        "INSERT INTO ops.freshness_metrics (table_name, freshness_seconds, max_loaded_at) SELECT 'fct_transactions', EXTRACT(EPOCH FROM (now() - max(_ingested_at)))::bigint, max(_ingested_at) FROM banking.bronze.transactions"
                    )
                )
