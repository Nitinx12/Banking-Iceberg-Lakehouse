...
def test_gx_watch_events_null_rejected(spark):
    ...
    df = spark.createDataFrame(
        [("e1", "u1", "c1", "play", "2024-01-01T00:00:00+00:00", 100, None, "s1")],
        schema=schema,
    )
...
def test_gx_watch_events_duplicate_rejected(spark):
    df = spark.createDataFrame(
        [
            ...
            ("e1", "u1", "c1", "play", "2024-01-02T00:00:00+00:00", 100, "tv", "s1"),
        ],
        schema=[
            "event_id",
            "user_id",
...
def test_gx_billing_null_amount(spark):
    ...
    df = spark.createDataFrame(
        [("t1", "u1", None, "USD", "charge", "2024-01-01T00:00:00+00:00", "basic")],
        schema=schema,
    )
