"""GX suite smoke — expectations files exist, valid JSON, and map to data_generator messiness."""
...
def test_subscriptions_suite_covers_cdc():
    suite = json.loads((pathlib.Path("gx/expectations/subscriptions_cdc.json")).read_text())
...
def test_billing_suite_covers_pk_and_range():
    ...
    assert any(e["kwargs"].get("column") == "transaction_id" and e["expectation_type"] == "expect_column_values_to_be_unique" for e in suite["expectations"])


def test_content_catalog_suite_row_count():
    suite = json.loads((pathlib.Path("gx/expectations/content_catalog.json")).read_text())
    assert any(e["expectation_type"] == "expect_table_row_count_to_be_between" for e in suite["expectations"])
